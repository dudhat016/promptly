  ...
  return (
    <div className={cn("relative w-full space-y-1.5", className)} ref={containerRef}>
      ...
      <div
        ...
        className={cn(
          "flex min-h-[44px] mb-0 w-full items-center justify-between rounded-md border bg-card px-4 py-1.5 text-base transition-colors cursor-pointer",
          // State-driven border: error > open/focus > idle
          error
      ...
      >
        <div className="flex flex-wrap gap-1.5 flex-grow">
          {isMulti ? (
            ...
            selectedOptions ? (
              <span className="text-foreground font-medium whitespace-nowrap">{(selectedOptions as SelectOption).label}</span>
            ) : (
              <span className="text-muted-foreground/60">{placeholder}</span>
            )
          )}
      ...
      {/* Error Message */}
      {error && (
        <p className="text-[11px] font-bold text-destructive px-1 animate-in fade-in slide-in-from-top-1">
          {error}
        </p>
      )}
      ...
      {isOpen && (
        <div 
          id={`${selectId}-listbox`}
          role="listbox"
          className="absolute z-[100] mt-1 w-full rounded-2xl border border-border bg-card shadow-2xl p-1 animate-in fade-in slide-in-from-top-2 duration-300 overflow-hidden"
        >
          {isSearchable && (
            <div className="p-2">
              <div className="relative">
                <input
                  ...
                  autoFocus
                  className="w-full bg-muted/50 border border-transparent rounded-xl pl-9 pr-4 py-2 text-base focus:border-primary focus:bg-card focus:outline-none transition-colors placeholder:text-muted-foreground/40"
                  placeholder="Search..."
                  value={searchTerm}
                  ...
                  autoComplete="off"
                />
                {/* Rendered after input so it paints on top of the input background */}
