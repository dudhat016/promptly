...
export default function ImageUpload({
  ...
  const inputId = `admin-upload-${generatedId}`;
    ...
    <div className={className}>
      <div className="flex flex-col gap-2 mb-2">
        {label && (
          <label 
            htmlFor={inputId}
            className="text-[11px] font-bold uppercase tracking-[0.15em] text-muted-foreground ml-1 cursor-pointer"
          >
            {label}
          </label>
        )}
      ...
      <div className="flex items-start gap-4">
        {value ? (
          ...
          <div 
            className={cn(
              "rounded-xl border-2 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-muted/50 hover:border-primary/40 transition-all text-muted-foreground hover:text-primary",
              ratioClass
            )}
          ...
          >
            {isUploading ? (
              ...
              <>
                ...
                <span className="text-xs font-bold uppercase tracking-widest mt-1">Upload</span>
              </>
            )}
          </div>
        )}
